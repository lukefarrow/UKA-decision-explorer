...
function render(){
 ...
 const r=revision[band][state.sex],baseURev=r[state.bearing],roboticFactor=state.robotic==='robotic'?0.83:1,uRev=baseURev*roboticFactor,tRev=r.tkr,s=safety(age),life=lifetime(age),uO=ukaOks(age,pre),tO=tkrOks(age,state.sex,bmi,+state.asa,pre);
 ...
 const roboticText=state.robotic==='robotic'
   ?'Robotic-assisted UKA estimate applies the AOANJRR adjusted revision HR of 0.83 to the NJR baseline. This is a transparent cross-registry estimate, not a validated combined model.'
   :'Manual UKA uses the unmodified NJR age × sex × bearing estimate.';
 ...
 <article class="outcome-card"><div class="eyebrow dark">Longevity</div><h3>10-year revision</h3><div class="compare">${metric(bearingLabel+(state.robotic==='robotic'?' + robotic':''),uRev.toFixed(1)+'%',`${band}, ${state.sex}`)}${metric('TKR',tRev.toFixed(1)+'%','cemented unconstrained fixed-bearing')}</div><div class="delta">Absolute excess revision with UKA: +${(uRev-tRev).toFixed(1)} percentage points.${state.robotic==='robotic'?` Robotic adjustment changes the UKA estimate from ${baseURev.toFixed(1)}% to ${uRev.toFixed(1)}%.`:''}</div><div class="outcome-copy">${provider} ${bearingText} ${roboticText}</div>${evidence('Revision risk varies materially by age, sex, implant construct and surgeon practice. UK NJR strata are used for the live 10-year estimate because they provide reproducible age × sex × bearing data. Australian, New Zealand, Swiss and US registry studies support the same broad pattern—higher UKA revision than TKR overall, strong age and provider effects, and construct-specific variation—but absolute rates differ enough that pooling would be misleading.',[{pmid:42448245,label:'2026 fixed vs mobile systematic review/meta-analysis'},{pmid:41442900,label:'Minimum 10-year randomized fixed vs mobile UKA trial'},{pmid:32114810,label:'AOANJRR robotic vs non-robotic UKA survivorship study'},{pmid:41749402,label:'2026 robotic vs conventional UKA revision meta-analysis'},{pmid:39147075,label:'Michigan registry: surgeon volume and 5-year UKA revision'},{pmid:35964854,label:'NJR: caseload, usage and 10-year mobile-bearing UKA survival'},{pmid:41270774,label:'TOPKAT 10-year UKA vs TKR comparison'}])}</article>
