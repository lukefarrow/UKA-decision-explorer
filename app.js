...
function render(){
 ...
 $('tradeoffRows').innerHTML=[
  ...
  tradeRow('Periprosthetic joint infection','Swiss registry HR 0.53','uka','Favours mUKA'),
  ...
  tradeRow('Return to activity','Activity','uka','Favours mUKA'),
 ...
 <article class="outcome-card"><div class="eyebrow dark">Pain & function · medial UKA evidence</div><h3>Oxford Knee Score at 12 months</h3><div class="compare">${metric('Medial UKA','41.2','95% CI 39.6–42.7; randomized trial')}${metric('TKR','38.4','95% CI 36.9–40.0; randomized trial')}</div><div class="delta">Matched 12-month adjusted improvement difference: +3.2 OKS points favouring medial UKA (95% CI 0.9–5.6).</div><div class="outcome-copy">This is a matched-timeframe randomized comparison in patients with isolated medial osteoarthritis. The previous 6-month UKA and 12-month TKR prognostic estimates are no longer displayed side-by-side because they used different models and follow-up times. Patient-specific OKS inputs remain available for future validated prognostic modelling.</div>${evidence('Recent randomized evidence reports at most a modest conventional PROM difference, while TOPKAT found no clinically important mean OKS difference at long-term follow-up.',[{pmid:41662451,label:'2026 blinded multicentre RCT'},{pmid:41270774,label:'TOPKAT 10-year randomized comparison'},{pmid:25042552,label:'Large UKA PROM prediction cohort'},{pmid:29467465,label:'Externally validated TKR OKS prediction model'}])}</article>

 <article class="outcome-card"><div class="eyebrow dark">Joint awareness</div><h3>Forgotten Joint Score</h3><div class="outcome-main">Favours mUKA</div><div class="delta">Typical comparative difference: about +6 to +14 FJS points with UKA, depending on follow-up.</div><div class="outcome-copy">This is a treatment-level comparative effect, not an individualized FJS forecast from the sliders.</div>${evidence('Comparative studies consistently report less joint awareness after UKA than TKR, with effect size varying by follow-up.',[{pmid:41825825,label:'2026 FJS meta-analysis'},{pmid:41662451,label:'2026 blinded multicentre RCT'}])}</article>

 <article class="outcome-card"><div class="eyebrow dark">Longevity</div><h3>10-year revision</h3><div class="compare">${metric(bearingLabel,uRev.toFixed(1)+'%',`95% CI ${rev.ukaCI[0].toFixed(1)}–${rev.ukaCI[1].toFixed(1)}%; ${band}, ${state.sex}`)}${metric('TKR',tRev.toFixed(1)+'%',`95% CI ${rev.tkrCI[0].toFixed(1)}–${rev.tkrCI[1].toFixed(1)}%; cemented unconstrained fixed-bearing`)}</div><div class="delta">Absolute excess revision with mUKA: +${(uRev-tRev).toFixed(1)} percentage points.</div><div class="outcome-copy">${provider} ${bearingText} ${roboticText}</div>${evidence('Revision risk varies materially by age, sex, implant construct and surgeon practice. UK NJR strata are used for the live 10-year estimate because they provide reproducible age × sex × bearing data. Earlier AOANJRR analyses suggested lower revision with robotic UKA, but the 2025 AOANJRR report found no adjusted revision difference for unicompartmental knee replacement. Robotics therefore does not alter the numeric estimate.',[{pmid:42448245,label:'2026 fixed vs mobile systematic review/meta-analysis'},{pmid:41442900,label:'Minimum 10-year randomized fixed vs mobile UKA trial'},{pmid:32114810,label:'AOANJRR robotic vs non-robotic UKA survivorship study'},{pmid:41749402,label:'2026 robotic vs conventional UKA revision meta-analysis'},{pmid:39147075,label:'Michigan registry: surgeon volume and 5-year UKA revision'},{pmid:35964854,label:'NJR: caseload, usage and 10-year mobile-bearing UKA survival'},{pmid:41270774,label:'TOPKAT 10-year UKA vs TKR comparison'}])}</article>

 <article class="outcome-card"><div class="eyebrow dark">Longevity</div><h3>Remaining-lifetime revision</h3><div class="compare">${metric('UKA','~'+Math.round(life.uka)+'%','research age-interpolated estimate')}${metric('TKR','~'+Math.round(life.tkr)+'%','research age-interpolated estimate')}</div><div class="delta">Approximate lifetime excess revision with UKA: ~${Math.round(life.uka-life.tkr)} percentage points.</div><div class="outcome-copy">${life.n}</div>${evidence('Age dominates remaining-lifetime revision exposure. Sex and ASA also modify risk, but no complete open age×sex×ASA competing-risk equation was identified for faithful individual calculation.',[{pmid:35638212,label:'NZ registry lifetime UKA revision risk'},{pmid:39631511,label:'NJR implant-specific lifetime revision modelling'}])}</article>

 <article class="outcome-card"><div class="eyebrow dark">Early postoperative complications</div><h3>30-day morbidity and mortality</h3><div class="compare">${metric('UKA',s.uka.toFixed(1)+'%',`95% CI ~${s.ukaCI[0].toFixed(1)}–${s.ukaCI[1].toFixed(1)}%; ${s.label}`)}${metric('TKR',s.tkr.toFixed(1)+'%',`95% CI ~${s.tkrCI[0].toFixed(1)}–${s.tkrCI[1].toFixed(1)}%; ${s.label}`)}</div><div class="delta">Approx. ${(s.tkr-s.uka).toFixed(1)} percentage points lower with UKA.</div><div class="outcome-copy">This outcome is the study's composite probability of 30-day morbidity or mortality / short-term postoperative complications from ACS-NSQIP. The source reports UKA without medial/lateral stratification, so it is supportive rather than strictly medial-specific evidence. It is not an overall measure of surgical “safety” and is not a comprehensive individualized perioperative-risk calculator. Confidence intervals between the published age anchors are interpolated for display.</div>${evidence('Large comparative datasets consistently favour UKA for early medical safety, although absolute risk depends on patient and pathway factors.',[{pmid:39233099,label:'Age-specific UKA vs TKA 30-day safety study'},{pmid:30792179,label:'Systematic review of patient-relevant UKA vs TKR outcomes'}])}</article>

 <article class="outcome-card"><div class="eyebrow dark">Infection</div><h3>Periprosthetic joint infection revision</h3><div class="compare">${metric('UKA','0.4%','observed SIRIS PJI revisions')}${metric('TKA','0.8%','observed SIRIS PJI revisions')}</div><div class="delta">Adjusted registry comparison: PJI-related revision HR 0.53 for UKA vs TKA.</div><div class="outcome-copy">Swiss national registry data (2012–2024) included 35,286 UKAs and 188,952 TKAs. The registry analysis did not stratify the UKA cohort by medial versus lateral compartment, so this is supportive UKA evidence rather than a medial-specific estimate. These crude percentages reflect available registry follow-up rather than a fixed-time cumulative risk, so the hazard ratio is the more appropriate comparative measure. Male sex and ASA ≥3 were associated with higher PJI revision risk in the registry.</div>${evidence('This recent national registry analysis provides a useful infection-specific comparison that complements the broader 30-day medical-safety outcome.',[{pmid:41779036,label:'2026 Swiss SIRIS national registry PJI analysis'},{pmid:35638212,label:'NZJR lifetime revision study: infection contribution to UKA vs TKA revisions'}])}</article>
